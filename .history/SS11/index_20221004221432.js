...
// let primeArr = numberArr.filter((item) => isPrime(item));
...
// let user = users.find(user => user.id === n)

// console.log(user)

// Nhập vào 1 chuỗi keyword. Hãy lấy ra email của các user có tên (first_name + last_name) chứa keyword
"NolanaIvory" 
let keyword = " noLana ".trim().toLowerCase()

let filterUser = users.filter(user => (user.first_name + user.last_name).trim().toLocaleLowerCase().includes(keyword))

console.log(filterUser.map(e => e.email))
