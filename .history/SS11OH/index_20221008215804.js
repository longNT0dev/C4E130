...
function logPosts(postList) {
  for (let post of postList) {
    let publisher = users.find((user) => user.id === post.user_id);
    console.log(`
            ID: ${post.id},
            Title: ${post.title},
            Posted At: ${post.created_at},
            Published: ${publisher.first_name} ${publisher.last_name}
        `);
  }
}

function findPostByUser(email) {
  if (!email) {
  ...
  let filterPost = posts.filter((post) => posts.user_id === user.id);

  console.log(filterPost);
}

let emailUserInput = "nduncanson9@harvard";
let passwordUserInput = "12345678";
let first_name = "Nguyễn";
let last_name = "Long";

// login(emailUserInput, passwordUserInput);
// signup(first_name, last_name, emailUserInput, passwordUserInput);
// searchUser("er");
// logPosts(posts);
findPostByUser("opilcher6@surveymonkey.com")
